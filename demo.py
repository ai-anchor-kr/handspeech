...
def load_checkpoints(config_path, checkpoint_path, avd_checkpoint_path, cpu=False):
    ...
    if cpu:
        ...
        avd_checkpoint = torch.load(avd_checkpoint_path, map_location=torch.device('cpu'))
    else:
        checkpoint = torch.load(checkpoint_path)
        avd_checkpoint = torch.load(avd_checkpoint_path)

    generator.load_state_dict(checkpoint['generator'])
    region_predictor.load_state_dict(checkpoint['region_predictor'])
    if 'avd_network' in avd_checkpoint:
        avd_network.load_state_dict(avd_checkpoint['avd_network'])
